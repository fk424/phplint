 ...
 * @version $Date: 2014/11/24 21:11:30 $
 */
final class Result implements Printable {
	...
	public static function implementsToString($x){
		// FIXME: __toString() implicit conversion allowed only since PHP 5.x
		if( ! $x instanceof ClassType )
			return FALSE;
		$c = cast(ClassType::NAME, $x);
		$m = $c->searchMethod(ClassMethod::$TO_STRING_NAME);
		if( $m === NULL )
			return FALSE;
		// FIXME: should account __toString() method usage? public methods are not reported as "unused" anyway!
//		$m->used++;
//		$m->class_->used++;
		return TRUE;
	}
