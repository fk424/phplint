 ...
 * @version $Date: 2014/11/24 21:17:22 $
 */
class Documentator {
...
"\n"
	...
	/**
	 * Writes an HTML link to another class.
	...
	private function docPackage(){
		...
		$this->write("<b>PHP Version:</b> $php_ver<br>\n");
		
		$this->write("<b>Required modules:</b>\n");
		...
		for($i = 0; $i < count($modules); $i++){
			if( $i > 0 )  $this->write(",\n");
			$this->write("<code>" . $modules[$i]->getName()->toASCII() . "</code>");
		}
		$this->write("<br>\n");
		
		$this->write("<b>Required packages:</b>\n");
		...
		if( $use_scrollbox ){
			$this->write("<div style='height:20em; width:100%; border:1px solid #ccc; overflow:auto;'>");
		}
	...
	private function writePrototype($name, $title, $sign){
		...
		for($i = 0; $i < count($sign->arguments); $i++){
			...
			if( ! $a->is_mandatory ){
				$this->write(" = ");
				$this->writeValue($a->type, $a->value);
			}
		}
